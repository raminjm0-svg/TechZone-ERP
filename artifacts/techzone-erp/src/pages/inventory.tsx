              ...
              {lowStockProducts.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {lowStockProducts.map(p => (
                    <div key={p.id} className={`flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border-l-4 ${p.stock === 0 ? 'border-rose-500' : 'border-amber-500'} border-y border-r border-y-border border-r-border`}>
                      <div className={`p-2 rounded-lg ${p.stock === 0 ? 'bg-rose-100' : 'bg-amber-100'}`}>
                        <AlertCircle className={`w-5 h-5 ${p.stock === 0 ? 'text-rose-600' : 'text-amber-600'}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="font-bold text-sm truncate pr-2">{p.name}</h4>
                          <span className={`text-xs font-bold ${p.stock === 0 ? 'text-rose-600' : 'text-amber-600'}`}>
                            {p.stock} left
                          </span>
                        </div>
                        <p className="text-xs text-muted-foreground">{p.category}</p>
                        <Progress value={(p.stock / 10) * 100} className={`h-1.5 mt-2 ${p.stock === 0 ? '[&>div]:bg-rose-500' : '[&>div]:bg-amber-500'}`} />
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="mt-4 flex gap-2 overflow-x-auto pb-2">
                    {categories.map(c => (
                      <button
                        ...
                        className={`whitespace-nowrap px-3 py-1 rounded-full text-sm font-semibold transition-colors ${categoryFilter === c ? 'bg-primary text-white' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'}`}
                      >
                        {c}
                      </button>
                    ))}
                          ...
                          <TableRow key={product.id} className="group hover:bg-secondary/20 transition-colors">
                            ...
                            <TableCell className="text-right py-4">
                              <Badge className={`px-2.5 py-1 text-xs font-bold border ${
                                product.stock > 10 ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 
                                product.stock > 0 ? 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100' : 
                            ...
                            <TableCell className="text-right px-6 py-4">
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-[160px]">
                                  <DropdownMenuItem onClick={() => toast({ title: "Edit Mode", description: `Editing ${product.name}` })}>
                                    <Pencil className="mr-2 h-4 w-4" />
                                    Edit Product
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => toast({ title: "Deleted", description: `${product.name} removed.` })} className="text-rose-600 focus:text-rose-600 focus:bg-rose-50">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input placeholder="e.g. Sony Corporation" className="h-11 rounded-xl" {...field} />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input placeholder="e.g. 4710820111049" className="h-11 rounded-xl" {...field} />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                          )}
