...
import { dashboardData, salesOrders } from '@/data/dummyData';
...
const ChartTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-border rounded-xl shadow-xl px-4 py-3 text-sm z-50">
...
export default function Dashboard() {
  /* show 8 most-recent orders from salesOrders */
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between">
        ...
        <span className="hidden sm:inline-flex items-center gap-1.5 text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 py-1.5 rounded-full">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
        ...
        {/* ── Row 2: Revenue Area Chart + Sales Donut ── */}
            ...
            <div className="flex items-start justify-between mb-6">
              ...
              <div className="flex items-center gap-5 text-xs font-semibold text-muted-foreground">
                <span className="flex items-center gap-2">
                  <span className="w-3 h-[3px] rounded-full bg-primary" /> Revenue
                </span>
                <span className="flex items-center gap-2">
                  <span className="w-3 h-[3px] rounded-full bg-blue-200 border-b border-dashed border-blue-300" /> Target
                </span>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={270}>
              <AreaChart data={revenueData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="month" axisLine={false} tickLine={false}
                  tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12, fontWeight: 500 }} dy={10} />
                ...
                <Area type="monotone" dataKey="target"  name="Target"
                  stroke="#93c5fd" strokeWidth={2} strokeDasharray="5 5" fill="none" dot={false} />
                <Area type="monotone" dataKey="revenue" name="Revenue"
                  stroke="hsl(var(--primary))" strokeWidth={3} fill="url(#gradRev)"
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={categoryBreakdown} cx="50%" cy="50%"
                    ...
                    stroke="none" labelLine={false} label={DonutLabel}>
                    {categoryBreakdown.map((c, i) => <Cell key={i} fill={c.color} />)}
                  </Pie>
                  <Tooltip
                    formatter={(v: number) => [`${v}%`, 'Share']}
                    contentStyle={{ borderRadius: 12, border: '1px solid hsl(var(--border))', fontSize: 12, fontWeight: 500, padding: '8px 12px' }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
        ...
        <motion.div variants={item} className="bg-white rounded-2xl shadow-sm border border-border p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={monthlySalesBar} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}
              barSize={14} barCategoryGap="30%">
              <CartesianGrid strokeDasharray="4 4" vertical={false} stroke="hsl(var(--muted))" />
              <XAxis dataKey="month" axisLine={false} tickLine={false}
                tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12, fontWeight: 500 }} dy={10} />
              <YAxis axisLine={false} tickLine={false}
              ...
              {Object.entries(BAR_COLORS).map(([cat, color]) => (
                <Bar key={cat} dataKey={cat} stackId="a" fill={color}
                  radius={cat === 'Accessories' ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          ...
          <motion.div variants={item}
            className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-border overflow-hidden flex flex-col">
            <div className="px-6 py-5 border-b border-border flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-foreground tracking-tight">Recent Orders</h2>
                <p className="text-xs font-medium text-muted-foreground mt-0.5">Latest 8 transactions</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold text-primary cursor-pointer hover:underline flex items-center gap-1">
          ...
          <motion.div variants={item}
            ...
            <div className="px-6 py-5 border-b border-border flex items-center justify-between">
              <div>
                ...
                <p className="text-xs font-medium text-muted-foreground mt-0.5">Items needing reorder</p>
              </div>
            ...
            <div className="px-6 pt-4 pb-2 flex items-center gap-3">
              <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200 px-3 py-1 rounded-full">
                ...
                {dashboardData.lowStockAlerts.filter(x => x.stock === 0).length} Out of Stock
              </span>
              <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1 rounded-full">
                ...
                {dashboardData.lowStockAlerts.filter(x => x.stock > 0).length} Low Stock
              </span>
            </div>
                  ...
                  <div key={item.id} className="px-6 py-4 hover:bg-muted/30 transition-colors">
                    <div className="flex items-start justify-between mb-2.5">
                      <div className="min-w-0 pr-3">
                        ...
                        <p className="text-xs font-medium text-muted-foreground mt-0.5">{item.category} · {item.id}</p>
                      </div>
